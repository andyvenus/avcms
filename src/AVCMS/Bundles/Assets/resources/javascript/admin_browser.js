...
avcms.browser = {
        ...
        if (response_data.form.has_errors === false) {
            ...
            if (id !== undefined) {
                $(form).find('*').filter(':input:not(button)').each(function(i, form_field){
                    ...
                    $('[data-id="'+id+'"][data-field='+field_name+'], [data-id="'+id+'"] > [data-field='+field_name+']').text(field_value).html();
                });
    ...
    finderLoadMore: function() {
        if(($(this).scrollTop() + ($(this).innerHeight() + 150) >= $(this)[0].scrollHeight) && (window.avcms.browser.finder_loading != 1)) {
