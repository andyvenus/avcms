...
// AVCMS
// Some code to automatically get the page title & url

$(document).ready(function() {
    var current_href = encodeURIComponent($(location).attr('href'));

    if (avcms.config.user_id) {
        if (current_href.indexOf('?') === -1) {
            current_href += '%3F'; // question mark
        }
        else {
            current_href += '%26'; // ampersand
        }

        current_href += 'ref_id='+avcms.config.user_id;
    }

    var current_title = encodeURIComponent($(document).attr('title'));

    $('.rrssb-buttons a').each(function() {
        var current_url = $(this).attr('href');

        var new_url = current_url.replace('{title}', current_title).replace('{url}', current_href);
